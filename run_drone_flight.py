...
class DroneFlightSession:
    ...
    def start(self) -> Dict[str, Any]:
        ...
        if self.last_observation is not None:
            save_color_observation(self.last_observation, self.run_dir / "step_0000_reset.png")
    ...
    def close(self) -> None:
        ...
        try:
            if env is not None and drone_name is not None:
                env.unwrapped.unrealcv.set_move_bp(drone_name, [0.0, 0.0, 0.0, 0.0])
                time.sleep(0.2)
        finally:
    ...
    def capture_observation(self, *, save: bool = False, label: str = "capture") -> Optional[np.ndarray]:
        ...
        if save and image is not None and self.run_dir is not None:
            save_color_observation(image, self.run_dir / f"step_{self.total_step:04d}_{label}.png")
        return image
