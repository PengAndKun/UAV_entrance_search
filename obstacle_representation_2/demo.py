...
class ObstacleRepresentation2Predictor:
    ...
    def predict(self, rgb_path: str | Path, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        mean = self.geometry_mean
        std = self.geometry_std
        if mean.shape != geometry.shape:
            mean = np.zeros_like(geometry, dtype=np.float32)
        if std.shape != geometry.shape:
            std = np.ones_like(geometry, dtype=np.float32)
        ...
        depth = torch.from_numpy(
            load_depth(str(event.get("depth_npy_path") or event.get("depth_path") or ""), self.image_size).copy()
        ...
        front_risk_state = self.labels[selected_idx] if 0 <= selected_idx < len(self.labels) else "clear"
        ...
        if front_stop_fraction >= 0.01 or (front_min > 0.0 and front_min <= 100.0) or must_stop_probability >= 0.5:
            ...
            front_warning_fraction >= 0.025
            and "obstacle_warning" in self.labels
            and self.labels.index(front_risk_state) < self.labels.index("obstacle_warning")
        ):
            front_risk_state = "obstacle_warning"
        elif (
            front_clearance_fraction >= 0.10
            and "clearance_warning" in self.labels
            and self.labels.index(front_risk_state) < self.labels.index("clearance_warning")
        ):
        ...
        return {
            ...
            "model_version": str(self.config.get("model_version", "a_plus_2_v1")),
...
def predict_obstacle_representation_2(
    ...
    *,
    ...
    return ObstacleRepresentation2Predictor(model_path, device_name=device_name).predict(rgb_path, event)
