...
def load_dataset_arrays(dataset_path: Path) -> Dict[str, np.ndarray]:
    ...
    for name in ("front_box_stop_fraction", "front_box_warning_fraction", "front_box_clearance_fraction"):
        if name in data.files:
            arrays[name] = data[name].astype(np.float32)
    if "risk_states" in data.files:
        arrays["risk_states"] = data["risk_states"].astype(str)
    if "projection_box" in data.files:
        arrays["projection_box"] = data["projection_box"].astype(np.float32)
    return arrays
...
def dice_loss_from_logits(logits: torch.Tensor, target: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    ...
    denom = torch.sum(prob + target, dim=(2, 3))
    ...
    *,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(training):
            ...
            loss_mask = F.binary_cross_entropy_with_logits(output["mask_logits"], mask) + dice_loss_from_logits(
                output["mask_logits"], mask
            )
...
def train_model(
    ...
    *,
    ...
    model_path = model_dir / "a_plus_3_model.pt"
    metrics_path = model_dir / "a_plus_3_training_metrics.json"
    ...
    checkpoint = {
        ...
        "config": {
            ...
            "warning_depth_cm": WARNING_DEPTH_CM,
            "clearance_depth_cm": CLEARANCE_DEPTH_CM,
            "projection_box_stop_threshold": float(PROJECTION_BOX["stop_fraction_threshold"]),
        },
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Obstacle Representation 3 A+3 model.")
