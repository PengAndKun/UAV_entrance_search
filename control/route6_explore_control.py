...
from .route6_test_planner_control import Route6TestPlannerControlMixin
...
class Route6ExploreControlMixin(Route6TestPlannerControlMixin):
    def ensure_route6_state(self) -> None:
        ...
        if not hasattr(self, "route6_map_movement_lock_event"):
            self.route6_map_movement_lock_event = threading.Event()
        ...
        if not hasattr(self, "route6_test_planner_point_status_label"):
        ...
        if not hasattr(self, "route6_test_planner_pose_after_id"):
            self.route6_test_planner_pose_after_id = None
        if not hasattr(self, "route6_test_planner_los_monitor_after_id"):
            self.route6_test_planner_los_monitor_after_id = None
        if not hasattr(self, "route6_test_planner_los_monitor_active"):
            self.route6_test_planner_los_monitor_active = False
        if not hasattr(self, "route6_test_planner_los_monitor_last_trigger_key"):
        ...
        if not hasattr(self, "route6_test_planner_pose_var"):
            self.route6_test_planner_pose_var = _route6_string_var("UAV x=n/a y=n/a z=n/a yaw=n/a")
        if not hasattr(self, "route6_test_planner_los_monitor_var"):
            self.route6_test_planner_los_monitor_var = _route6_string_var("LOS Monitor: idle")
        if not hasattr(self, "route6_test_planner_point_status_var"):
            self.route6_test_planner_point_status_var = _route6_string_var("Observation Points: n/a")
    ...
    def route6_refresh_realtime_uav_pose(self, session: Any = None) -> Dict[str, float]:
        self.ensure_route6_state()
        active_session = session if session is not None else getattr(self, "session", None)
        if active_session is not None and callable(getattr(active_session, "get_state", None)) and bool(getattr(active_session, "started", True)):
            try:
            ...
            if isinstance(state, dict) and state:
                applied = False
                apply_state = getattr(self, "apply_state", None)
        ...
        return self.route6_current_pose()

    def route6_format_uav_pose_text(self, pose: Dict[str, Any]) -> str:
        item = pose if isinstance(pose, dict) else {}
        return (
            f"UAV x={float(item.get('x', 0.0) or 0.0):.1f}cm "
            f"y={float(item.get('y', 0.0) or 0.0):.1f}cm "
            f"z={float(item.get('z', 0.0) or 0.0):.1f}cm "
            f"yaw={float(item.get('yaw', item.get('yaw_deg', 0.0)) or 0.0):.1f}deg"
        )
    ...
    def route6_movement_allowed(self) -> bool:
        ...
        if getattr(self, "route6_full_stop_event", None) is not None and self.route6_full_stop_event.is_set():
            return False
        if getattr(self, "route6_map_movement_lock_event", None) is not None and self.route6_map_movement_lock_event.is_set():
            return False
    ...
    def route6_update_map_realtime_worker(
        ...
        *,
        ...
        while not self.route6_update_map_realtime_stop_event.is_set():
            loop_count += 1
            current_pose = self.route6_refresh_realtime_uav_pose(session)
    ...
    def route6_stop_map_capture_and_lock_movement(self, session: Any = None) -> Dict[str, Any]:
        self.ensure_route6_state()
        active_session = session if session is not None else getattr(self, "session", None)
        self.route6_update_map_capture_stop_event.set()
        self.route6_update_map_realtime_stop_event.set()
        self.route6_map_movement_lock_event.set()
        try:
            stop_keyboard = getattr(self, "stop_keyboard_control", None)
            if callable(stop_keyboard):
                stop_keyboard(send_hold=True, force_hold=True)
        except Exception:
            pass
        hold_result: Dict[str, Any] = {"status": "skipped", "reason": "missing_session"}
        movement_disable_result: Dict[str, Any] = {"status": "skipped", "reason": "missing_session"}
        if active_session is not None:
            if callable(getattr(active_session, "move_relative", None)):
                try:
                    hold_payload = action_payload("hold") if callable(globals().get("action_payload")) else {"action_name": "hold"}
                    hold_result = active_session.move_relative(hold_payload)
                except Exception as exc:
                    hold_result = {"status": "failed", "error": str(exc)}
            if callable(getattr(active_session, "set_movement_enabled", None)):
                try:
                    movement_disable_result = active_session.set_movement_enabled(False)
                except Exception as exc:
                    movement_disable_result = {"status": "failed", "error": str(exc)}
        self.movement_enabled_state = False
        try:
            ...
            pass
        record = {
            "schema": "route6_map_stop_lock_v1",
            "status": "stopped_and_locked",
            "capture_stop_requested": True,
            "realtime_stop_requested": True,
            "movement_lock_active": bool(self.route6_map_movement_lock_event.is_set()),
            "hold_result": self.route6_json_safe(hold_result),
            "movement_disable_result": self.route6_json_safe(movement_disable_result),
            "created_at": datetime.now().isoformat(timespec="milliseconds"),
        }
        self.llm_route6_state["route6_map_stop_lock"] = self.route6_json_safe(record)
        self.route6_write_state_artifact()
        self.route6_update_map_status_var.set(
            "Route 6 Update Map: stopped capture/realtime and disabled UAV movement. Clear releases the Route 6 movement gate; re-enable movement before flying."
        )
        if hasattr(self, "route6_test_planner_status_var"):
            self.route6_test_planner_status_var.set("Route 6 Test Planner: stopped map updates and locked UAV movement.")
        if hasattr(self, "llm_route6_status_var"):
            self.llm_route6_status_var.set("LLM Route V6: map capture stopped; UAV movement locked.")
        return record

    def on_route6_update_map_stop_and_lock_movement(self) -> Dict[str, Any]:
    ...
    def open_route6_update_map_window(self) -> None:
        ...
        toolbar.grid_columnconfigure(16, weight=1)
        ...
        tk.Button(toolbar, text="Stop Map + Lock Movement", command=self.on_route6_update_map_stop_and_lock_movement).grid(row=0, column=15, sticky="w", padx=6, pady=6)
        ...
        tk.Label(toolbar, textvariable=self.route6_update_map_status_var, anchor="w").grid(row=1, column=0, columnspan=17, sticky="ew", padx=6, pady=6)
        tk.Label(toolbar, textvariable=self.route6_update_map_pose_var, anchor="w").grid(row=2, column=0, columnspan=17, sticky="ew", padx=6, pady=(0, 6))
    ...
    def open_llm_route_window6(self) -> None:
        ...
        tk.Button(realtime_toolbar, text="Stop Map + Lock Movement", command=self.on_route6_update_map_stop_and_lock_movement).pack(side="left", padx=6)
    ...
    def on_route6_clear(self) -> None:
        ...
        self.route6_map_movement_lock_event.clear()
        self.route6_test_planner_los_monitor_active = False
        self.route6_test_planner_los_monitor_last_trigger_key = ""
        self.route6_test_planner_los_monitor_var.set("LOS Monitor: idle")
        self.route6_test_planner_point_status_var.set("Observation Points: n/a")
        self.llm_route6_thread = None
        self.llm_route6_state = {}
