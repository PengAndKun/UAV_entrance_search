...
from obstacle_avoidance_3.or2_direction_rule import DEFAULT_METHOD_ID, select_or2_direction
...
class ObstacleAvoidance3ControlMixin:
    ...
    def build_obstacle_avoidance_3_window(self, window: tk.Widget) -> None:
        ...
        self.obstacle_avoidance_3_environment_combo = ttk.Combobox(project, textvariable=self.obstacle_avoidance_3_environment_var, state="readonly", width=30)
        self.obstacle_avoidance_3_environment_combo.grid(row=1, column=1, sticky="ew", padx=6, pady=5)
        ...
        self.obstacle_avoidance_3_method_combo.grid(row=1, column=3, sticky="ew", padx=6, pady=5)
        ...
        columns = ("on", "episode_id", "start", "goal", "environment", "method", "hint", "note")
        tree = ttk.Treeview(episodes, columns=columns, show="headings", height=11, selectmode="extended")
        widths = (55, 80, 210, 210, 150, 250, 140, 200)
        labels = ("On", "ID", "Start [x,y,z,yaw]", "Goal [x,y,z,yaw]", "Environment", "Method", "Hint", "Note")
        ...
        runner.grid(row=4, column=0, sticky="ew", padx=8, pady=4)
        ...
        records.grid(row=5, column=0, sticky="ew", padx=8, pady=4)
        ...
        report_frame.grid(row=6, column=0, sticky="nsew", padx=8, pady=(4, 8))
    ...
    def load_obstacle_avoidance_3_plans(self) -> None:
        ...
        self.obstacle_avoidance_3_status_var.set(f"OA3 plans loaded: {self.obstacle_avoidance_3_plan_path()}")
    ...
    def pose_text(self, pose: Any) -> str:
        try:
        ...
        for episode in project.get("episodes", []):
            ...
            iid = tree.insert(
                ...
                values=(
                    ...
                    episode_id,
                    self.pose_text(episode.get("start_pose", "")),
                    self.pose_text(episode.get("goal_pose", "")),
    ...
    def load_obstacle_avoidance_3_episode_into_editor(self, episode: Dict[str, Any]) -> None:
        ...
        self.obstacle_avoidance_3_start_pose_var.set(self.pose_text(episode.get("start_pose", "")))
        self.obstacle_avoidance_3_goal_pose_var.set(self.pose_text(episode.get("goal_pose", "")))
    ...
    def apply_obstacle_avoidance_3_algorithm(self) -> None:
        ...
        status = "ready" if method == DEFAULT_METHOD_ID and model_path.is_file() else "blocked"
        self.obstacle_avoidance_3_status_var.set(f"OA3 algorithm {status}: {method}")
        self.obstacle_avoidance_3_report(
            {
    ...
    def execute_obstacle_avoidance_3_to_goal(self) -> None:
        self.run_obstacle_avoidance_3_current_episode()
    ...
    def start_obstacle_avoidance_3_runner(self, episodes: List[Dict[str, Any]], *, selected_label: str) -> None:
        ...
        project = self.current_obstacle_avoidance_3_project()
        run_id = sanitize_id(
            f"{project.get('project_id', DEFAULT_PROJECT_ID)}_{method}_{selected_label}_{datetime.now().strftime('%H%M%S')}",
            "oa3_run",
        )
        note = (
        ...
        self.obstacle_avoidance_3_stop_event = threading.Event()
        self.obstacle_avoidance_3_status_var.set(f"OA3 runner starting: {selected_label}")
        self.obstacle_avoidance_3_report(
            {
                "status": "starting",
    ...
    def run_obstacle_avoidance_3_collection_on_session(
        ...
        *,
        ...
        batch_dir = data_root / "route_episode_batches" / f"{timestamp}_route_episode_3_{method}_{run_id}"
        ...
        for episode_index, episode in enumerate(episodes, start=1):
            ...
            session_dir = data_root / "sessions" / f"{timestamp}_route_episode_3_{episode_id}_{method}_{run_id}"
            ...
            for frame_id in range(1, int(args.max_ticks_per_episode) + 1):
                ...
                try:
                    ...
                    try:
                        rgb_image = Image.open(str(event.get("rgb_path", ""))).convert("RGB")
                ...
                self.root.after(
                    0,
                    lambda eid=episode_id, fid=frame_id, dist=post_distance, reached=post_reached, completion=completion_reason, action=selected_action, risk=event.get("or2_front_risk_state", ""), hit=bool(event.get("collision_state")), reason=event.get("selected_action_reason", reason): (
