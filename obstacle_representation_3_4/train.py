...
def default_training_paths(output_root: Path | str = Path("obstacle_representation_3_data")) -> Dict[str, Path]:
    ...
    return {
        ...
        "model": root / "models" / "a_plus_3_4_model.pt",
        "metrics": root / "models" / "a_plus_3_4_training_metrics.json",
    }


def default_tversky_config() -> Dict[str, Any]:
    return {
        "learning_rate": 2e-4,
        "tversky_alpha": 0.3,
        "tversky_beta": 0.7,
        "tversky_gamma": 0.75,
        "stop_tversky_loss_weight": 1.5,
        "stop_mask_loss_weight": 2.5,
        "must_stop_loss_weight": 4.5,
        "distillation_weight": 0.08,
        "risk_class_weights": {
            "clear": 1.0,
            "clearance_warning": 1.15,
...
def focal_tversky_loss_from_logits(logits: Any, target: Any, *, alpha: float, beta: float, gamma: float, eps: float = 1e-6) -> Any:
    import torch

    prob = torch.sigmoid(logits)
    target = target.float()
    tp = torch.sum(prob * target, dim=(1, 2, 3))
    fp = torch.sum(prob * (1.0 - target), dim=(1, 2, 3))
    fn = torch.sum((1.0 - prob) * target, dim=(1, 2, 3))
    tversky = (tp + eps) / (tp + float(alpha) * fp + float(beta) * fn + eps)
    return torch.pow(1.0 - tversky, float(gamma)).mean()
    ...
    *,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(training):
            ...
            loss_stop_tversky = focal_tversky_loss_from_logits(
                output["mask_logits"][:, 2:3],
                mask[:, 2:3],
            ...
            loss = (
                loss_mask
                + float(config["stop_tversky_loss_weight"]) * loss_stop_tversky
                + loss_risk
                + 0.2 * loss_can_forward
                + 0.5 * loss_must_stop
                + float(config["distillation_weight"]) * loss_distill
            )
            if training:
                loss.backward()
                optimizer.step()
...
def train_model_3_4(
    ...
    *,
    ...
    for epoch in range(1, int(epochs) + 1):
        train_metrics = _run_epoch(model, teacher_model, train_loader, optimizer=optimizer, device=device, config=config)
        val_metrics = _run_epoch(model, teacher_model, val_loader, optimizer=None, device=device, config=config)
        history.append({"epoch": epoch, "train": train_metrics, "val": val_metrics})
        score = float(val_metrics["risk_accuracy"]) + 0.4 * float(val_metrics["must_stop_accuracy"]) + 0.2 * float(val_metrics["mask_iou"])
        if score > best_score:
            best_score = score
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Obstacle Representation 3.4 focal-Tversky model.")
