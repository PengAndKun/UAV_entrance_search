...
def default_training_paths(output_root: Path | str = Path("obstacle_representation_3_data")) -> Dict[str, Path]:
    ...
    return {
        ...
        "model": root / "models" / "a_plus_3_3_model.pt",
        "metrics": root / "models" / "a_plus_3_3_training_metrics.json",
    }


def default_focal_config() -> Dict[str, Any]:
    return {
        "learning_rate": 2e-4,
        "risk_focal_gamma": 2.0,
        "must_stop_focal_gamma": 2.0,
...
def effective_number_class_weights(counts: np.ndarray, *, beta: float = 0.999) -> np.ndarray:
    ...
    weights = (1.0 - beta) / np.maximum(effective, 1e-8)
    weights = weights / np.maximum(np.mean(weights), 1e-8)
    return weights.astype(np.float32)


def focal_cross_entropy(logits: Any, target: Any, class_weights: Any, *, gamma: float = 2.0) -> Any:
    import torch
    ...
    pt = torch.exp(log_pt).clamp(min=1e-6, max=1.0)
    return (((1.0 - pt) ** float(gamma)) * ce).mean()


def focal_bce_with_logits(logits: Any, target: Any, *, pos_weight: Any, gamma: float = 2.0) -> Any:
    import torch
    from torch.nn import functional as F
    ...
    return (((1.0 - pt) ** float(gamma)) * bce).mean()
    ...
    *,
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(training):
            ...
            loss_risk = focal_cross_entropy(
                output["risk_logits"],
                risk,
                class_weights,
                gamma=float(config["risk_focal_gamma"]),
            )
            loss_can_forward = F.binary_cross_entropy_with_logits(output["can_forward_logits"], can_forward)
            loss_must_stop = focal_bce_with_logits(
                output["must_stop_logits"],
                must_stop,
                pos_weight=pos_weight,
                gamma=float(config["must_stop_focal_gamma"]),
            )
...
def train_model_3_3(
    ...
    *,
    ...
    class_weights_np = effective_number_class_weights(risk_counts, beta=float(config["class_balance_beta"]))
    sampler_weights = np.asarray(
        [float(class_weights_np[int(idx)]) / max(1.0, risk_counts[int(idx)]) for idx in arrays["risk_indices"][train_indices]],
        dtype=np.float32,
    )
    ...
    for epoch in range(1, int(epochs) + 1):
        train_metrics = _run_epoch(model, teacher_model, train_loader, optimizer=optimizer, device=device, config=config, class_weights=class_weights)
        val_metrics = _run_epoch(model, teacher_model, val_loader, optimizer=None, device=device, config=config, class_weights=class_weights)
        history.append({"epoch": epoch, "train": train_metrics, "val": val_metrics})
        score = float(val_metrics["risk_accuracy"]) + 0.4 * float(val_metrics["must_stop_accuracy"]) + 0.15 * float(val_metrics["mask_iou"])
        if score > best_score:
            best_score = score
    ...
    checkpoint = {
        ...
        "config": {
            ...
            "focal_config": config,
            "class_weights": class_weights_np.astype(float).tolist(),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Obstacle Representation 3.3 focal/class-balanced model.")
